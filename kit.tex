...
\begin{defn}
  An \textbf{end} of a module $M\in \modk(A,A\o,\Psi)$ is a module $\tend^A M\in\modk(\Psi)$ together with a morphism $\chi\in\mhom{\big(\tend^A M\big)(\xi)}{M(a,a,\xi)}$, postcomposition with which induces bijections
  \[ \mhom{\Gamma(\gamma)}{(\tend^A M)(\theta)} \toiso \mhom{\Gamma(\gamma)}{M(a,a,\theta)} \]
  whenever this makes sense.
\end{defn}
