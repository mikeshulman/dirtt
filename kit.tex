...
\autodefs{\cMod\dCat\bCat\dSpan\dMod\bSet}
...
This is analogous to how proarrow equipments~\cite{wood:proarrow-i} and Yoneda structures~\cite{sw:yonstr} provide abstract contexts for doing one-variable formal category theory.
...
As observed by Day and Street~\cite{ds:monbi-hopf}, the opposite of a category is its dual in the monoidal bicategory of profunctors, making the latter ``compact closed'' (also called ``rigid'' or ``autonomous''); and inside a compact closed bicategory one can define structures of multivariable category theory.
...
\item A kit is ``fully virtual'' in the sense of~\cite{cs:genmulti}; that is, all of its structure is multicategorical.
...
Let \bbK be an equipment (that is, a double category in which every vertical arrow has a companion and a conjoint, or equivalently in which the (source,target) map is a fibration~\cite{shulman:frbi}), and let $T$ and $S$ be monads on \bbK in the sense of~\cite{cs:genmulti}.
...
Recall from~\cite{gp:something} that a \emph{horizontal transformation} between (pseudo double) functors $F,G$ consists of a horizontal arrow $\alpha_X  F X \to G X$ for each object $X$ of the domain, forming a pseudonatural transformation between the horizontal parts of $F$ and $G$, together with for each vertical arrow $f:X\to Y$ in the domain, a cell
\[ \xymatrix{ F X \ar[r]|-{|}^-{\alpha_X} \ar[d]_{F f} \ar@{}[dr]|{\alpha_f} & G X \ar[d]^{G f} \\
F Y \ar[r]|-{|}_-{\alpha_Y} & G Y} \]
satisfying some evident axioms.
These are the horizontal arrows in a double category whose vertical arrows are vertical transformations, and whose 2-cells are an evident kind of ``square modification''.
...
\begin{defn}\label{def:hdl}
  ...
  \begin{enumerate}
    ...
    \item Square modifications
      \[ \xymatrix{ T T S \ar[r]|{|}^{T \delta} \ar[d]_{\mu S} \ar@{}[drr]|{\hat\delta} &
        T S T \ar[r]|{|}^{\delta T} & S T T \ar[d]^{S \mu} \\
        T S \ar[rr]|{|}_{\delta}&& ST}\]
      \[ \xymatrix{ T S S \ar[r]|{|}^{\delta S} \ar[d]_{T \nu} \ar@{}[drr]|{\check\delta} &
...
\begin{defn}
If $\delta$ is a horizontal distributive law as above, then there is a \textbf{horizontal bi-Kleisli virtual equipment} defined as follows:
\begin{enumerate}
  ...
  \item A horizontal arrow from $X$ to $Y$ is a horizontal arrow $T X \hto S Y$ in \bbK.
    The cells with unary source are obvious.
  \item A cell with nullary source is a cell in \bbK like
    \[ \xymatrix{ & Z \ar[dl]_{f} \ar[dr]^g \\
      X \ar[d]_{\eta_X} & \Downarrow & Y \ar[d]^{\iota_Y} \\
      T X \ar[rr]|{|} && S Y } \]
  \item A cell with binary source is a cell in \bbK like
    \[ \xymatrix{ TTX \ar[r]|{|}^{T M} \ar[d]_{\mu_X} \ar@{}[ddrrr]|{\Downarrow} &
      ...
      T X' \ar[rrr]|{|} &&& S Z'}\]
  \item For cells with ternary or higher source, we need to choose, arbitrarily, a way to ``associate'' the $\delta$s.
    Here is where we need the fact that $\delta$ is horizontally pseudonatural, to ensure that the result is independent, up to isomorphism, of the choice.
    For definiteness, we choose to pull all the $S$'s to the left ``as soon as possible''.
    Thus, a cell with ternary source is a cell in \bbK like
    \[ \xymatrix{ TTTX \ar[r]|{|}^{TTM} \ar[d]_{\mu^2} \ar@{}[ddrrrrr]|{\Downarrow} &
      TTSY \ar[r]|{|}^{\delta^2} & STTY \ar[r]|{|}^{STN} & STSZ \ar[r]|{|}^{S\delta} & SSTZ \ar[r]|{|}^{SSP} & SSSW \ar[d]^{\nu^2}\\
  ...
  \item \label{item:hbkve-comp} Composition uses the four extra cell modifications that come along with $\delta$, and (in most cases) the horizontal pseudonaturality of $\delta$.
    ...
    \[ \xymatrix{ TTTX \ar[r]|{|}^{TTM} \ar[d]_{\mu T} \ar@{}[dr]|{\mu M} &
      ...
      TX \ar[d] &&&&&& SW'\ar[d]\\
      TX' \ar[rrrrrr]|{|} &&&&&& S W''}\]
    and this, where at the top we use the horizontal pseudonaturality of $\delta$ to make the source match up with our arbitrary choice of pulling all the $S$'s to the front first (if we made the opposite choice, then the pseudonaturality of $\delta$ would be required in the preceding diagram):
    \[ \xymatrix {
      TTTX \ar[r]|{|}^{TTM} \ar@{=}[d] &
      ...
      TX' \ar[d] &&&&&& SW\ar[d]\\
      TX'' \ar[rrrrrr]|{|} &&&&&& S W'}\]
    The axioms for $\delta$ and its associated cells ensure that these operations are suitably associative and unital.
\end{enumerate}
\end{defn}

It would admittedly take some technical work to formally define the cells with $n$-ary source for all $n$, all of their composition operations, and prove associativity and unitality.
However, since our real intent is only to give a definition, and (as we will see in a moment) the definition in question only requires cells of nullary, unary, and binary sources and their composites with at most ternary begin, we will omit that work for the present.
Here is the first step towards that definition:

\begin{defn}
  If $\delta$ is a horizontal distributive law $TS \hto ST$ as above, then a \textbf{$\delta$-monoid} is a monoid in the above horizontal bi-Kleisli virtual equipment.
\end{defn}
...
\[ \xymatrix{ TTA \ar[r]|{|}^{T M} \ar[d]_{\mu_A} \ar@{}[drrr]|{\Downarrow} &
  ...
  T A \ar[rrr]|{|}_M &&& S A}\]
such that the two ways to compose the multiplication with itself (as shown in~\ref{item:hbkve-comp} above) are equal, and moreover the two following two ways to compose the multiplication with the unit are equal to the identity on $M$:
...
\[ \xymatrix{ & TA \ar[dl]_{\eta_{TA}} \ar[r]|{|}^{M} &
  ...
  T S A \ar[r]|{|}^{\delta} & S T A \ar[r]|{|}^{S M} & S S A \ar[d]^{\nu_A}\\
  T A \ar[rrr]|{|}_M &&& S A}\]
Our definition will be a special case of $\delta$-monoids, and we will have no use for the remainder of the structure of the horizontal bi-Kleisli virtual equipment.
...
\begin{eg}
  If $T$ is the identity monad, then for any $S$ there is a canonical distributive law $TS \hto ST$ consisting of horizontal units, and $\delta$-monoids for this $\delta$ are precisely $S$-monoids in the sense of~\cite{cs:genmulti}.
\end{eg}
...
We showed in~\cite{cs:genmulti} that for generalized multicategories, in many situations these methods produce equivalent results, and argued that in cases when they disagree it is the second solution that is often preferable.
...
Expecting similar results for generalized polycategories, we extend the terminology of~\cite{cs:genmulti} as follows:
...
In~\cite{cs:genmulti} we justified the similar terminology ``virtual $S$-algebra'' by showing that pseudo $S$-algebras embed into virtual $S$-algebras and can be characterized therein by a notion of representability.
...
(For instance, when $\delta$ is as in \cref{eg:polycats}, the pseudo $\delta$-algebras should be the linearly distributive categories.)
...
This isomorphism, together with the pairings of objects in $B_2\leftrightarrow B_2\o$ and $D_1 \leftrightarrow D_1\o$, defines a graph that according to the rule of~\cite{ek:gfc} must have no loops.
