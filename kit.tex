...
\autodefs{\cMod\dCat\bCat\dSpan\cSpan\dMod\bSet\bGraph\dMod\cMod\fSpan\cDbl\cLx}
\let\Mod\dMod
\let\mod\cMod
\def\modk{\mod_\K}
\def\mor#1{\hom_{#1}}
...
% In~\cite{gp:forthcoming} this problem is solved by assuming that basic cells are ``transportable'' along transversal isomorphisms, such as the unit constraints for vertical composition.
% We will instead assume that vertical composition is strictly unital; this makes things simpler, and is actually more general since it can be achieved by redefining vertical composition if basic cells are transportable.
% (In many examples such as \dCat, vertical composition is defined only by a universal property, so we might as well choose it to be strictly unital anyway.)

% For simplicity, we will also always assume our intercategories are \textbf{normal}, meaning that all the nullary interchangers are isomorphisms, as is the case in $\fSpan(\dCat)$.
...
By definition, its loose morphisms are\dots
