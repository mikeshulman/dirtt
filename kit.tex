...
Consider functors $d:D\to C$ and $l:()\to C$, and a module $J\in\modk(D)$.
...
If the kit $\K$ has ends and function modules, an equivalent characterization of limits is possible:
$l$ is the $J$-weighted limit of $d$ if there is an isomorphism
\[ \mor{C}(c,l) \cong J' \multimap_D \mor{C}(c,d') \]
in the category $\modk(C\o)$.
...
In particular, if $l$ has unary domain $E$, then $l:E\to C$ is a functor given by ``pointwise'' limits.

As a special case, we can define (pointwise) Kan extensions.
If $f:D\to C$ and $g:D\to E$ are functors, then the right Kan extension of $f$ along $g$ (if it exists) is the limit of $f$ weighted by $\mor{E}(e,g(d))$.
