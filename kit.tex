...
The symmetric groups also act on modules; thus if $M\in \modk(A,B\o,C)$ we have a $\sigma^*M\in \modk(B\o,C,A)$ and so on.
...
Thus, representing these multicategorically we have multivariable module morphisms such as $(M_1,M_2,\dots,M_n) \to N$, where $M_i\in \modk(\Psi_i)$ and $N\in\modk(\Psi_1 \Psi_2\cdots \Psi_n)$.
As with functors, the symmetric groups act on the domains of such morphisms.

However, in order to really do multivariable category theory, we also need ``extraordinary'' natural transformations as in~\cite{ek:gen-funct-calc}.
For instance, given $M\in\modk(A,A\o,B)$ and $N\in \modk(C,C\o,B)$ we want to speak about a transformation $\alpha:M\to N$ that is ordinary natural in $B$ and extraordinary natural in $A$ and $C$ on either side; in plain set-based category theory this means we have components $\alpha_{a,b,c}:M(a,a,b) \to N(c,c,b)$ such that the following naturality squares commute for all $f:a_1\to a_2$, $g:b_1\to b_2$, $h:c_1\to c_2$, and $a,b,c$:
\begin{mathpar}
...
Here the $\Psi_i$ are lists of cats \emph{with variance}, and $\Psi\o$ denotes reversal of all the variances in $\Psi$.
...
And we can have multivariable extraordinary morphisms as well (see below).

The complication comes when defining how extraordinary morphisms can be \emph{composed}.
...
Of course, in practice it could be that the cats for which we want to assert ordinary or extraordinary naturality are not lined up in this exact order; for instance we could have $M\in \modk(A,B,A\o)$ and $N\in \modk(B,C,C\o)$.
Formally, in this case we have to use the symmetric group actions to put the cats in the right order; but in practice it is easier to just indicate how the cats should be matched up.
...
\end{mathpar}
This notation, or something like it, is also essential when talking about multivariable extraordinary morphisms.
...
However, it is useful to know that the notation can be made much more precise, by defining a type theory to serve as the ``internal language'' of a kit; see~\cite{lnss:dirtt}.
...
\end{mathpar}
The problem is that when ``composing components'' there is no way to decide what $y$ and $z$ should be: we can write down the composite
...
One of the simplest universal constructions in a kit is the tensor product of modules, whose universal property is very much like that of tensor products in an ordinary multicategory.
The extra wrinkle is that the universal property applies to all sorts of extraordinary morphisms.

\begin{defn}
  A \textbf{tensor product} of $M_1\in\modk(\Psi_1)$ and $M_2\in\modk(\Psi_2)$ is a module $M_1\otimes M_2 \in \modk(\Psi_1\Psi_2)$ along with an (ordinary) morphism $\chi:(M_1,M_2)\to M_1\otimes M_2$ such that precomposition with $\chi$ induces bijections
  \[ \mhom{\Gamma,M_1\otimes M_2}{N} \toiso \mhom{\Gamma,M_1,M_2}{N} \]
  for any list $\Gamma$ of modules and any ``variable pairing'' such that this makes sense.
\end{defn}

This universal property includes extraordinary morphisms of all sorts, where paired variables may occur anywhere.
Examples include:
\begin{align*}
  \mhom{M_1(a) \otimes M_2(b)}{N(a,b)} &\toiso \mhom{M_1(a), M_2(b)}{N(a,b)}\\
  \mhom{M_1(a) \otimes M_2(a)}{N} &\toiso \mhom{M_1(a), M_2(a)}{N}\\
  \mhom{P(a),M_1(a,b) \otimes M_2(c)}{N(b,c,d,d)} &\toiso \mhom{P(a),M_1(a,b), M_2(c)}{N(b,c,d,d)}
\end{align*}
and so on.
Note that due to the symmetric group actions, it suffices to assert this universal property when $M_1\otimes M_2$ is at the end of the domain list; a corresponding property then follows when it appears anywhere.

Similarly, we can define $n$-ary tensor products for all $n$, and the usual arguments show that these are associative and symmetric insofar as they exist; i.e. if $M\otimes N$ and $(M\otimes N)\otimes P$ exist, then the latter is a ternary tensor product; hence if $N\otimes P$ exists then $M\otimes (N\otimes P)$ exists and is isomorphic to $(M\otimes N)\otimes P$.
A nullary tensor product is a module $I\in \modk(())$ and a morphism $() \to I$ precomposition with which induces bijections $\mhom{\Gamma,I}{N} \toiso \mhom\Gamma N$.

We can similarly talk about tensor products of cats; now the extra wrinkle is that the universal property has to apply to modules as well as to functors.
...
\begin{defn}
  ...
  \item Precomposition with $\chi$ induces bijections of functor sets
    \[ \K((\Psi,A\otimes B),C) \toiso \K((\Psi,A,B),C)\]
    for any list $\Psi$ of cats with variance;
  \item Restriction along $\chi$ induces an isomorphism of categories
    \[ \modk(\Psi,A\otimes B) \toiso \modk(\Psi,A,B) \]
    for any $\Psi$; and finally
  \item Restriction along $\chi$ induces bijections of sets of extraordinary morphisms
    \[ \mhomc{\Psi,A\otimes B}{\Gamma}{N}\toiso \mhomc{\Psi,A,B}{\chi^*\Gamma}{\chi^*N}\]
    whenever this makes sense.\label{item:tenscat-3}
  \end{enumerate}
\end{defn}
In \ref{item:tenscat-3} we have omitted the variables that would normally be assigned to the cats.
If we wrote instead $(z:A\otimes B)$ and $(x:A),(y:B)$ then instead of $\chi^*N$ we ought to write $N[\chi(x,y)/z]$, meaning to substitute $\chi(x,y)$ for $z$ wherever it appears in $N$ (and in $\Gamma$); but we will try to avoid this more type-theoretic sort of notation as much as possible.

The internalization of contravariance looks essentially the same.

\begin{defn}
  An \textbf{opposite} of a cat $A$ is a cat $A\op$ with a functor $\chi:(A\o) \to A\op$ precomposition with which induces isomorphisms
  \begin{align*}
    \K((\Psi,A\op),C) &\toiso \K((\Psi,A\o),C)\\
    \modk(\Psi,A\op) &\toiso \modk(\Psi,A\o) \\
    \mhomc{\Psi,A\op}{\Gamma}{N} &\toiso \mhomc{\Psi,A\o}{\chi^*\Gamma}{\chi^*N}
  \end{align*}
\end{defn}
