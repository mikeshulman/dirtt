...
We will define a loose distributive law on $V\C$ in $\cL(\dCat)$, inducing one on $\dCat/V\C$ whose generalized polycategories are ``unary kits without functors''.

The two monads involved in this distributive law will in fact be the same monad, which we denote $R$.
Being a horizontal monad in $\cL(\dCat)$, it is equivalently a monad in the bicategory $\cSpan(\bCat)$, i.e.\ an internal category in \bCat, i.e.\ a \emph{strict} double category $\dR$.
