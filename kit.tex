...
\def\rightcircarrowfill@{%
  \slashedarrowfill@\relbar\relbar{\raisebox{1pt}{$\scriptscriptstyle\circ$}}\rightarrow}
...
\autodefs{\cMod\dCat\bCat\dSpan\dMod\bSet\bGraph\dMod\cMod\fSpan\cDbl\cLx}
\let\Mod\dMod
\let\mod\cMod
\def\modk{\mod_\K}
\def\mor#1{\hom_{#1}}
...
In particular, their underlying functors are given by span composition; functors between slice categories of this sort are sometimes called \textbf{linear} (by analogy with the more general \emph{polynomial} functors that also involve an exponential).
...
For this it is convenient to have a structure that describes ``spans in \dCat''.
...
It has three kinds of arrow, called \emph{transversal}, \emph{horizontal}, and \emph{vertical}; three kinds of cell, called \emph{horizontal} (horizontal/transversal), \emph{vertical} (vertical/transversal), and \emph{basic} (horizontal/vertical); and also \emph{cubes}.
...
Moreover, composition of basic cells satisfies the interchange law (in both binary and nullary versions) only up to a \emph{not-necessarily-invertible} transversal cube.

It is shown in~\cite[\S6]{gp:intercategories-ii} that there is an intercategory $\fSpan(\dCat)$ whose:
\begin{itemize}
...
\item Vertical cells are profunctor morphisms (cells in \dCat),
\item Horizontal cells are span morphisms (cells in the double category $\dSpan(\bCat)$),
\item Basic cells are spans of profunctor morphisms, and
\item Cubes are morphisms between spans of profunctor morphisms.
\end{itemize}
...
The nullary interchangers are isomorphisms, but the binary-binary one is not in general.
...
To enhance this to a version of \cref{thm:intercat-bicat}, it is natural to use the notion of \emph{locally cubical bicategory} from~\cite{gg:lowdim-tricats}.
These are categories weakly enriched over the cartesian monoidal category of double categories, \emph{pseudo} double functors, and tight transformations.
Thus, they have objects, morphisms, tight and loose 2-cells, and square 3-cells.

\begin{lem}
  Let \iA be a normal intercategory with strictly unital vertical composition.
  Then there is a locally cubical bicategory $\cL(\iA)$ whose objects are those of \iA and whose hom-double-categories are the ``tightly full'' sub-double-categories of $\hom_\iA(A,B)$ containing only those basic cells of \iA that are pseudo for postcomposition (and hence only those horizontal arrows of \iA whose vertical identities are such).
  Moreover, for any object $X$ there is a locally cubical pseudofunctor $\hom_\iA(X,-):\cL(\iA) \to \cDbl$, where $\cDbl$ denotes the tautological locally cubical bicategory of double categories, strong double functors, tight transformations, and loose transformations.
\end{lem}
\begin{proof}
  TODO.
\end{proof}

All this would be in vain if no cells were pseudo for postcomposition, but fortunately we have some.
For instance:

\begin{lem}
  A basic cell in $\fSpan(\dCat)$, consisting of a span of cells in $\dCat$:
  \[ \xymatrix{ A \ar@{<-}[r] \ar@{=}[d] \ar@{}[dr]|{\Leftarrow} & 
    B \ar[r] \ar[d]|{\bullet} \ar@{}[dr]|{\Rightarrow}& 
    C \ar[d]|{\bullet} \\
    A \ar@{<-}[r] & D \ar[r] & E } \]
  is pseudo for postcomposition if $A$ is a groupoid and the functors $B\to A$ and $D\to A$ are fibrations.
\end{lem}
\begin{proof}
...
To see this, simply take the definition and replace functors by 1-cells, tight and loose transformations by tight and loose 2-cells, square modifications by square 3-cells, and so on.
The pseudonaturality of $\dl$ comes from the invertible interchange, which in the locally cubical world corresponds to the composition functor being strong rather than lax.

It follows that we can also define loose distributive laws in any intercategory, by working with globular cells and adding the condition that they are pseudo for postcomposition.
...
For the rest of this section, let \C
