...
\begin{array}{l}
d_1 = v_1 \combine v_1' \\ 
d_2 = v_2 \combine v_2' \combine v_2'' \\ 
\Psi^{\flip{v_1}} \types \Gamma_1 \ctx \\ 
\Psi^{v_1'} \types M \type \\ 
\Psi^{\flip{v_2}} \types \Gamma_2 \ctx \\
\Psi^{\flip{v_2'}} \types \rho : \Psi^{v_1'} \\
\Psi^{v_2''} \types N \type \\
\end{array}
\]

Therefore, $\Psi^{v_1 \combine v_2 \combine v_2''} \types
(\Gamma_1,\Gamma_2 \vdash N) \ok$.  

So the unification needs to produce a context $\Psi_0$ and a renaming
$\Psi_0 \vdash \rho_0 : \Psi^{v_1 \combine v_2 \combine v_2''}$.
