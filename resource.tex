...
(FIXME: check that it's associative and commutativity).  
...
\inferrule{v = {v_1} \combine {({\flip {p_1}} \combine p_1)} \combine {({\flip {p_2}} \combine p_2)} \\
           \Psi^{v_1}, x \mcol A, y \pcol A \cb (\Gamma \vdash M) \ok \\
           \Psi^{p_1} \vdash a_1 : A \\
           \Psi^{p_2} \vdash a_2 : A \\
           \Delta, x \pmcol A \cb (\Gamma \vdash M) [x/y]\\ 
           }
          {\Psi^v \cb (\Gamma, \mor{A}(x,y) \types M)[ (a,a)/x\pm , (b,b)/y\pm] }
\end{mathpar}

Two-sided without a substitution:
\begin{mathpar}
\inferrule*[right=derivable]
           {\Delta, x \mcol A, y \pcol A \cb (\Gamma \vdash M) \ok \\
            \Delta, x \pmcol A \cb (\Gamma \vdash M) [x/y]\\ }
...
\inferrule*[right=admissible using cut?]
           {v = {v_1} \combine {({\flip {p_1}} \combine p_1)} \combine {({\flip {p_2}} \combine p_2)} \\\\
...
\begin{mathpar}
  ...
  \inferrule{
    v = v_1 \combine v_2 \\\\ 
    \Psi^{v_1} \cb (\Gamma_1 \vdash M_1) \ok \\
    \Psi^{v_1} \cb \Gamma_1 \vdash M_1 \\\\
    \Psi^{v_2} \cb (\Gamma_2 \vdash M_2) \ok \\
    \Psi^{v_2} \cb \Gamma_2 \vdash M_2
  }{
    \Psi^{v} \cb \Gamma_1,\Gamma_2 \types M_1\otimes M_2
  }\and
\end{mathpar}
...
\begin{mathpar}
  ...
  \inferrule{
    \Delta, x \pmcol A \cb \Gamma, M \types N
    }{
    \Delta \cb \Gamma,\coend_{x:A} M \types N
  }\and
...
\begin{mathpar}
  ...
  \inferrule{
    \Delta, x \pmcol A \cb \Gamma \types M
    }{
...
\begin{mathpar}
  ...
  \inferrule{
    ...
    \Psi^{v_1} \cb (\Gamma_1 \vdash M_1) \ok \\ 
    \Psi^{v_1} \cb \Gamma_1 \vdash M_1 \\\\
    \Psi^{v_2} \cb (\Gamma_2,M_2 \vdash N) \ok \\ 
    \Psi^{v_2} \cb \Gamma_2,M_2 \vdash N \\ 
  }{
    \Psi^v \cb \Gamma_1,\Gamma_2,M_1\multimap M_2 \types N
  }
\end{mathpar}
