...
\newcommand\triv{\_}
\newcommand\forgetpandm{\downarrow^{\ps\ms}}
\newcommand\forgetpm{\downarrow^{\pms}}
\newcommand\complete{\uparrow}
...
We will also need operations that drop things
\[
\begin{array}{lll}
\forgetpm \pms & = & \uns \\
\forgetpm r & = & r \text{ otherwise } \\
\\ 
\forgetpandm \ps & = & \uns \\
\forgetpandm \ms & = & \uns \\
\forgetpandm r & = & r \text{ otherwise } \\
\end{array}
\]


We write $\Psi^{v}$ to indicate that ${v}$ is the vector of variances on
the assumptions in $\Psi$, and write $\combine$, $\flip{}$, $\forgetpm$,
$\forgetpandm$ for the pointwise operation on vectors.  When $x$ comes
from a context $\Psi$, we write $x^v$ for the vector whose $x$ component
is $v$ and is $\uns$ otherwise.  When we write an $r$ in place of a
vector, that means repeat $r$ the appropriate number of times.

We write $p$ for a vector containing no $\pms$'s, and $d$ for a vector
containing only $\pms$ and $\uns$.  Note that $\flip{p} \combine p$ is
always defined and is a $d$.  We write $\complete v := v \combine
\flip{(\forgetpm v)}$ for the vector where every $\ps$ or $\ms$ in $v$
has been replaced by a $\pm$.
...
\and
\inferrule*[right=admissible? (not used yet)]
           {\Psi \vdash \rho' : \Psi'}
           {\Psi' \vdash \rho'^{-1} : \Psi}
\end{mathpar}
...
\inferrule{\Psi^{v_1} \vdash \theta : \Psi'  \\
           ...
           {\Psi^{v} \vdash (\theta,t/x) : (\Psi',x \vcol r A)}
\end{mathpar}
...
(\theta,t/x) = (\theta_1,t_1/x) \combine (\theta_2,t_2/x) \text{ when } \theta = \theta_1 \combine \theta_2
\text{ and } t = t_1 \combine t_2 \\
\\ 
t = (t \combine \triv) \text{ if the splitting is } r = r \sqcup \uns\\
t = (\triv \combine t) \text{ if the splitting is } r = \uns \sqcup r\\
...
(FIXME: non-$\uns$?) $r$.  Under the prespposition that $\Psi^{v_1}
\vdash \rho : \Psi^{v_2}$ (i.e. $\rho$ is an isomorphism between two
parts of $\Psi$) and $x,y \in \Psi$ , we define the relation $x
\samestring y [ \rho ]$, which means $x$ and $y$ are on the same string
when the two parts of $\Psi$ are glued together by $\rho$.  We write
$\rho(x,y)$ to mean $y/x$ or $(y,z)/x$ or $(z,y)/x$ is one of the
substitutions in $\rho$.  
...
\inferrule*[right=admissible?]
           { d = d_1 \combine d_2 \and 
             ...
             \Psi^{\flip{v_2}} \types (\Gamma_2 \types N) \ok \and
             \Psi^{\flip{v_2'}} \types \rho : \Psi^{v_1'} \\\\
             \Psi^{d_1} \cb \Gamma_1 \vdash M \and 
             \Psi^{d_2} \cb \Gamma_2, M[\rho] \vdash N \\\\
             \Psi^{\flip{\forgetpm v_1}} \types \rho_{12} : \Psi^{\forgetpm{v_2}} \and
             (\forall x,y \in \Psi, \rho_{12}(x,y) \: \supset \: x \samestring y [\rho] ) \and
             (\forall x \in \Psi^{v_1' \combine v_2'}. \exists y \in \Psi^{v_1}. x \samestring y [\rho] ) \and
           }
           {\Psi^{(\complete v_1) \combine (\forgetpandm v_2)} \cb (\Gamma_1,\Gamma_2 \vdash N)[1_{v_1} \combine \rho' \combine 1_{\forgetpandm {v_2}}]}
\end{mathpar}

FIXME: this is too restrictive: something can start and end on the same
side.  

The first two lines are just the preconditions for the two terms being
cut together on the third line: $d_1$ and $d_2$ are two independent
parts of $\Psi$, which each are split, with some resources going towards
$M$ and the the leftovers going towards the rest of the sequent.
Because the cut formula $M$ occurs in two different contexts, we use a
renaming $\rho$ to match up the variables.  

In the conlusion of the cut, the context includes (a) all $\pms$
variables from $v_1$, and (b) all $\pms$ variables from $v_2$, and (c)
all variables from $v_1$ that were either $\ps$ or $\ms$, now promoted
to $\pms$.  The idea is that the non-$\pms$ variables from $v_1$ and
$v_2$ are now paired up, and we are choosing (somewhat arbitrarily) to
use the sames from $v_1$ for them.  Formally, recall that $\forgetpandm
v_2$ keeps only the $\pms$ variables, and $\complete v_1$ = $v_1
\combine \flip{\forgetpm{v_1}}$ (where $\forgetpm{v_1}$ keeps only the
$\ps$ and $\ms$ variables) completes $v_1$ with every $\ps$ or $\ms$
becoming a $\pm$.  The renaming $\rho_{12}$ matches up the $\ps$ or
$\ms$ variables from $v_2$ with the variables from $v_1$ that they will
be paired with.

A $\rho_{12}$ satisfying the conditions should be unique if it exists.

Algorithmically, given the $\rho$, we compute a $\rho'$ option by
computing the equivalence classes of $x \samestring y [ \rho ]$,
and then testing the conditions.  FIXME: check/explain
...
\begin{mathpar}
  \inferrule{
    d = d_1 \combine d_2 \\
    \Psi^{d_1} \types M_1\type \\ \Psi^{v_2} \types M_2\type
  }{
    \Psi^{d} \types M_1\otimes M_2\type
  }\and
  \inferrule{
...
\begin{mathpar}
  ...
  \inferrule{ d = d_1 \combine (\flip{p_x} \combine p_x) \combine (\flip{p_y} \combine p_y) \combine (\flip p \combine p) \\\\
              \Psi^p \vdash a : A}
            { \Psi^d \cb \Gamma \types \coend^{w:A} M }
...
\begin{mathpar}
  ...
  \inferrule*[right=derivable?]
             { \Psi, x \pmcol A, y \pmcol A \cb (\Gamma \types M) [(x,y)/z, (y,x)/w ]}
