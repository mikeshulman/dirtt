...
\begin{mathpar}
  ...
  \inferrule{
    \unif{\Psi^{d_1=v_1 \combine v_1'}}{\Psi^{(d_2\combine x\pm) = (v_2 \combine x\pm) \combine v_2'}}{\rho[1_{v_1 \combine v_2} \combine x/x \combine x/y]}{(\Psi^d \vdash \rho_0)}
  }{
    \unif{\Psi^{d_1\combine z\pm = v_1 \combine (v_1' \combine z\pm)}}
         {\Psi^{d_2\combine x\pm\combine y\pm = (v_2 \combine (x\m \combine y\p)) \combine (v_2' \combine (x\p \combine y\m))}}
         {(\rho \combine (x,y)/z)}
         {(\Psi_d \vdash (1_{v_1 \combine v_2} \combine x/x \combine x/y)[\rho_0])}
  }
...
FIXME: missing some possibilities?
...
inputs, and $d,\rho_0$ as outputs.  FIXMEL: when can the rules fail?
...
\combine x/y) : \Psi^{x^{\flip b} \combine y^{b}}$ does this.  FIXME:
missing cases when both $x$'s are internal and when both $y$'s are
internal?  

Rule 3: FIXME missing cases when $x$ is internal and/or $y$ is internal?
FIXME: does it matter whether we replace both both $x$ and $y$ with $x$
or with $y$?
