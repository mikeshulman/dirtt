...
\inferrule{d = {d_1} \combine {({\flip {p_1}} \combine p_1)} \combine {({\flip {p_2}} \combine p_2)} \\
          ...
          {\Psi^d \cb (\Gamma, \mor{A}(x,y) \types M)[ (a_1,a_1)/x\pm , (a_2,a_2)/y\pm] }
\end{mathpar}
...
There are four cases of the hom left rules, depending on how the variables $x$ and $y$ split between
$\Gamma$ and $M$. Below are versions of three of these four cases, first with a built in cut on the
left, then with cuts on both sides:
\begin{mathpar}
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\pcol A,y\mcol A \types \Gamma' \ctx \\
    \Psi^{v_2'} \types M \type \\\\
    \Psi^{v_3} \types a_1:A \\
    \Psi^{\flip{v_3'}} \types a_2:A \\
    d_1 = v_1 \combine v_1' \combine v_3 \combine v_3' \\
    d_2 = v_2 \combine v_2' \\\\
    \Psi^{d_1} \cb \Gamma \types \Gamma'[a_1/x,a_2/y] \\
    \Psi^{d_2},x\pmcol A \cb \Gamma'[\rho,x/y] \types M \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
    \unif{\Psi^{d_1=(v_1\combine v_3\combine v_3')\combine v_1'}}{\Psi^{d_2=v_2'\combine v_2}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0 \cb (\Gamma,\mor{A}(a_1,a_2) \types M)[\rho_0]
  } \and
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\pcol A \types \Gamma' \ctx \\
    \Psi^{v_2'},y\pcol A \types M \type \\\\
    \Psi^{v_3} \types a_1:A \\
    \Psi^{p_2} \types a_2:A \\
    d_1 = v_1 \combine v_1' \combine v_3 \\
    d_2 = v_2 \combine v_2' \\\\
    \Psi^{d_1} \cb \Gamma \types \Gamma'[a_1/x] \\
    \Psi^{d_2},x\pmcol A \cb \Gamma'[\rho] \types M[x/y] \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
    \unif{\Psi^{d_1=(v_1\combine v_3)\combine v_1'}}{\Psi^{d_2=v_2'\combine v_2}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0,\Psi^{p_2\combine\flip{p_2}} \cb (\Gamma,\mor{A}(x,y) \types M)[\rho_0,a_1/x,(a_2,a_2)/y]
  } \and
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\mcol A, y\pcol A \types M \type \\\\
    \Psi^{p_1} \vdash a_1 : A \\
    \Psi^{p_2} \vdash a_2 : A \\
    d = v_1\combine v_1' \\\\
    \Psi^{d},x\pmcol A \cb \Gamma \types M[x/y] \\
    }{
    \Psi^{d\combine p_1\combine\flip{p_1}\combine p_2\combine\flip{p_2}}
      \cb (\Gamma, \mor{A}(x,y) \types M)[(a_1,a_1)/x, (a_2,a_2)/y]
  }
\end{mathpar}
and with cuts on both sides (can these be written as a single rule?):
\begin{mathpar}
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\pcol A,y\mcol A \types \Gamma' \ctx \\
    ...
    \Psi^{d_2},x\pmcol A \cb \Gamma'[\rho,x/y] \types M \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
    \unif{\Psi^{d_1=(v_1\combine v_3\combine v_3')\combine v_1'}}
      {\Psi^{d_2=v_2'\combine v_2}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0 \cb (\Gamma,\mor{A}(a_1,a_2) \types M)[\rho_0]
  } \and
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},y\pcol A \types \Gamma' \type \\
    \Psi^{v_2'} \types M \type \\\\
    \Psi^{v_3} \types a_1:A \\
    \Psi^{\flip{v_3'}} \types a_2:A \\
    d_1 = v_1 \combine v_1' \combine v_3 \\
    d_2 = v_2 \combine v_2' \combine v_3' \\\\
    \Psi^{d_1} \cb \Gamma \types \Gamma'[a_1/y] \\
    \Psi^{d_2} \cb \Gamma'[\rho,a_2/y] \types M \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
    \unif{\Psi^{d_1=(v_1\combine v_3)\combine v_1'}}
      {\Psi^{d_2=(v_2\combine v_3')\combine v_2'}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0 \cb (\Gamma,\mor{A}(a_1,a_2) \types M)[\rho_0]
  } \and
  \inferrule{
    ...
    \Psi^{\flip{v_3'}} \types a_2:A \\
    d_1 = v_1 \combine v_1' \\
    d_2 = v_2 \combine v_2' \combine v_3 \combine v_3' \\\\
    \Psi^{d_1},x\pmcol A \cb \Gamma \types \Gamma'[x/y] \\
    \Psi^{d_2} \cb \Gamma'[\rho,a_1/x,a_2/y] \types M \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
    \unif{\Psi^{d_1=v_1\combine v_1'}}
      {\Psi^{d_2=(_2'\combine v_3\combine v_3')\combine v_2}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0 \cb (\Gamma,\mor{A}(a_1,a_2) \types M)[\rho_0]
  }
\end{mathpar}

The following based rules are special cases of the middle rule above (and the similar unwritten one):
\begin{mathpar}
...
The left rule for end should be all substitutions/cuts with the left axiom, which gives (FIXME:
check):
\begin{mathpar}
  \inferrule*[right=End Left Rule]{
    ...
    d_1 = v_1\combine v_1' \\
    d_1 = v_2\combine v_2' \\\\
    \Psi^{v_1\combine v_2} \types (\Gamma,\End_{x:A} M \types N) \ctx \\\\
    \Psi^{v_1'} \types a:A \\
    \Psi^{\flip{v_2'}} \types \rho:\Psi^{v_1'} \\\\
    \Psi^d \cb \Gamma, \End_{x:A} M[(a,a[\rho])/x] \types N \\\\
    \unif{\Psi^{d_1=v_1\combine v_1'}}{\Psi^{d_2=v_2\combine v_2'}}{\rho}{(\Psi_0,\rho_0)}
    }{
    \Psi_0 \cb (\Gamma, \End_{x:A} M \types N)[\rho_0]
  }
\end{mathpar}
