...
$\Gamma$ and $M$. Below are versions of three of these four cases with a built in cut on the left
(FIXME: it is probably possible to rewrite this without needing a morphism of type contexts):
\begin{mathpar}
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\pcol A,y\mcol A \types \Gamma_1 \ctx \\
    \Psi^{v_2'} \types M \type \\\\
    \Psi^{v_3} \types a_1:A \\
    \Psi^{\flip{v_3'}} \types a_2:A \\
    d_1 = v_1 \combine v_1' \combine v_3 \combine v_3' \\
    d_2 = v_2 \combine v_2' \\\\
    \Psi^{d_1} \cb \Gamma \types \Gamma_1[a_1/x,a_2/y] \\
    \Psi^{d_2},x\pmcol A \cb \Gamma_1[\rho,x/y] \types M \\\\
    ...
    \Psi_0 \cb (\Gamma,\mor{A}(x,y) \types M)[\rho_0,a_1/x,a_2/y]
  } \and
  \inferrule{
    \Psi^{\flip{v_1}} \types \Gamma \ctx \\
    \Psi^{v_1'},x\pcol A \types \Gamma_1 \ctx \\
    \Psi^{v_2'},y\pcol A \types M \type \\\\
    \Psi^{v_3} \types a_1:A \\
    \Psi^{p_2} \types a_2:A \\
    d_1 = v_1 \combine v_1' \combine v_3 \\
    d_2 = v_2 \combine v_2' \\\\
    \Psi^{d_1} \cb \Gamma \types \Gamma_1[a_1/x] \\
    \Psi^{d_2},x\pmcol A \cb \Gamma_1[\rho] \types M[x/y] \\\\
    \Psi^{\flip{v_2}} \types \rho:\Psi^{v_1'} \\
