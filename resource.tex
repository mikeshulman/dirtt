...
\newcommand\triv{\_}
\newcommand{\unif}[4]{#1\doteq #2\,\text{via }#3\cb #4}
\mprset{flushleft}
...
Before we can state a cut rule, we introduce a new judgement and rules for deriving valid string
diagram compositions. The new judgement has the form $\unif{\Psi^{d_1}}{\Psi^{d_2}}{\rho}{\rho_0}$,
where $d_1=v_1\combine v_1'$, $d_2=v_2\combine v_2'$, $\Psi^{\flip{v_2'}} \types \rho:\Psi^{v_1'}$,
and $\Psi^d \types \rho_0:\Psi^{v_1\combine v_2}$. This should be read as stating that $\Psi^d$ is
the result of composing $\Psi^{d_1}$ and $\Psi^{d_2}$ by gluing along $\rho$, and that the
composition introduced no loops.
