...
variances pointwise.  When $x,y,\ldots$ come from a context $\Psi$, we
write $(x^v,y^{v'},\ldots)$ for the vector whose $x$ component is $v$
and whose $y$ component is $v'$, and so on, and is $\times$ otherwise.
...
\and
\inferrule*{\Psi^{v_2} \vdash \rho : \Psi'  \\
             v_1 = {v_2} \combine (y^v) 
           }
           {\Psi^{v_1} \vdash (\rho,y/x) : (\Psi',x \vcol r A)}
\and
\inferrule*{\Psi^{v_2} \vdash \rho : \Psi'  \\
             v_1 = {v_2} \combine (y\m,z\p) 
           }
           {\Psi^{v_1} \vdash (\rho,(y,z)/x) : (\Psi',x \pmcol A)}
\and
...
The first rule gives \emph{strengthening} for $x \uncol A$ --- we can
supply nothing for it, because it can never be used.  Dually, the
renaming that gives weakening when applied to a judgement should be
admissible for the rest of the rules.  The renaming for the empty
context is well-formed if all of the assumptions have already been
spent.  The renaming for a context extension with $x \vcol r A$ says
that you need to spend $v$ from an assumption $y$ in the context, with
whatever is left over being used in the rest of the renaming.  For
example, we have
\[
...
substituting two different variables for its contravariant and covariant
aspects.  In all of the judgements $\Psi \vdash J$, $x \pmcol A$ is
...
\inferrule{\Psi^{v_1} \vdash \theta : \Psi'  \\
           ...
           {\Psi^{v} \vdash (\theta,a/x) : (\Psi',x \pcol A)}
\and
\inferrule*{\Psi^{v'} \vdash \theta : \Psi'  \\
             \Psi^{\flip{v_1}} \vdash {a_1} : A \\
             \Psi^{v_2} \vdash {a_2} : A \\
             v = {v'} \combine {({v_1} \combine {v_2})} \\
           }
