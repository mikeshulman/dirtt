...
\inferrule*{\Psi \vdash \rho : \Psi' }
           {\Psi \vdash (\rho,\triv/x) : (\Psi', x \uncol A)}
\and 
\inferrule*{\Psi^{v'} \vdash \rho : \Psi' \and 
             v = v' \combine y\p
           }
           {\Psi^{v} \vdash (\rho,y/x) : (\Psi', x \pcol A)}
\and
\inferrule*{\Psi^{v'} \vdash \rho : \Psi' \and 
             v = v' \combine y\m
           }
           {\Psi^{v} \vdash (\rho,y/x) : (\Psi', x \mcol A)}
\and 
\inferrule*{\Psi^{v'} \vdash \rho : \Psi'  \\
             v = {v'} \combine y\m \combine z\p 
           }
           {\Psi^{v} \vdash (\rho,(y,z)/x) : (\Psi',x \pmcol A)}
\and
...
\inferrule{\Psi \vdash \theta : \Psi' }
           {\Psi \vdash (\theta,\triv/x) : (\Psi', x \uncol A)}
\and
\inferrule{\Psi^{v_1} \vdash \theta : \Psi'  \\
            \Psi^{v_2} \vdash a : A \\
             v = {v_1} \combine v_2
           }
           {\Psi^{v} \vdash (\theta,a/x) : (\Psi',x \pcol A)}
\and 
\inferrule{\Psi^{v_1} \vdash \theta : \Psi'  \\
            \Psi^{\flip{v_2}} \vdash a : A \\
             v = {v_1} \combine v_2
           }
           {\Psi^{v} \vdash (\theta,a/x) : (\Psi',x \pcol A)}
\and
\inferrule*{\Psi^{v_1} \vdash \theta : \Psi'  \\
             \Psi^{\flip{v_{21}}} \vdash {a_1} : A \\
             \Psi^{v_{22}} \vdash {a_2} : A \\
             v = {v_1} \combine {({v_{21}} \combine {v_{22}})} \\
           }
           {\Psi^{v} \vdash (\theta,(a_1,a_2)/x) : (\Psi',x \pmcol A)}
\end{mathpar}
...
\inferrule*[right=derivable?]
           ...
           {\Psi^{v} \vdash (\theta,a/x) : (\Psi',x \pmcol A)}
\end{mathpar}

There will need to be some operations decomposing a substitution along a
splitting $v = v_1 \combine v_2$, and some operation applying $*$
functorially to substitutions, which probably switches $(a_1,a_2)$ to
$(a_2,a_1)$?
...
\end{mathpar}
We write $\theta \splits \theta_1,\theta_2$ for this operation when the
...
\begin{mathpar}
\uns = \uns \combine \uns \and \inferrule*{ }{\cdot \splits \cdot,\cdot}
\end{mathpar}
...
\begin{mathpar}
(\theta,t/x) \splits (\theta_1,?/x),(\theta_2,?/x) \\
v = (v_{11} \combine ?) \combine ({v_{21} \combine ?}) \\
\Psi^{(v_{11} \combine ?)} \vdash (\theta_1,?/x) : (\Psi'^{v_1'},x \vcol{r_1'} A)\\
\Psi^{(v_{21} \combine ?)} \vdash (\theta_1,?/x) : (\Psi'^{v_2'},x \vcol{r_2'} A)
\end{mathpar}
where the two goals split $v_2$, because we already know $v = v_1
\combine v_2$, so by associativity and commutativity, we will have 
$v = v_1 \combine v_2 = (v_{11} \combine v_{12}) \combine (? \combine ?)
= (v_{11} \combine ?) \combine ({v_{21} \combine ?})$.  

Writing $\Psi^{v_2} \vdash t \vcol{r'} A$ for the remaining premises of
the substitution rule ($t$ might really be zero or two terms), the lemma
we need is 
\begin{mathpar}
\inferrule{r' = r_1' \combine r_2' \\ 
...
\end{mathpar}
which we write as $t \splits t_1,t_2$ because then we can define
\begin{mathpar}
(\theta,t/x) \splits (\theta_1,t_1/x),(\theta_2,t_2/x)
\end{mathpar}
and satisfy the above.  We define this by cases on how $r'$ splits:

\begin{itemize}
\item Case $r' = r' \combine \uns$.  In this case return $v_2 = v_2
  \combine \uns$ and $t \splits (t,\triv)$.
\item Case $r' = \uns \combine r'$.  In this case return $v_2 = \uns
  \combine v_2$ and $t \splits (\triv,t)$.
\item Case $\pms = \ps \combine \ms$. In this case $(a_1,a_2) \splits a_2,a_1$.
\item Case $\pms = \ms \combine \ps$. In this case $(a_1,a_2) \splits a_1,a_2$.
\end{itemize}
...
\begin{mathpar}
  \inferrule{
    \Psi^{v_1} \types \Gamma \ctx\\
    \Psi^{v_2} \types M \type\\
    v = \flip{v_1} \combine v_2
  }{
    \Psi^{v} \cb (\Gamma \vdash M) \ok
  }
\\ 
\end{mathpar}
...
TODO: substitution rule for $\Delta \cb \Gamma \types M$ (only allow
same thing for both halves).  
...
\begin{mathpar}
\inferrule{ \Psi^{v_1} \types a : A \and  
...
\inferrule*{v = {v_1} \combine {({\flip {p_1}} \combine p_1)} \combine {({\flip {p_2}} \combine p_2)} \\\\
            \Psi^{(v_1 \combine \flip{p_1})},x \pcol A \cb (\Gamma \vdash M) \ok \\\\
            \Psi^{p_1} \vdash {a_1} : A \\\\
            \Psi^{p_2} \vdash {a_2} : A \\\\
            \Psi^{(v_1 \combine \flip{p_1} \combine {p_1})} \cb (\Gamma \vdash M)[a_1/x] }
          {\Psi^{v} \cb (\Gamma, \mor{A} (a_1,x) \vdash M)[a_2/x]}

\inferrule*{v = {v_1} \combine {({\flip {p_1}} \combine p_1)} \combine {({\flip {p_2}} \combine p_2)} \\\\
            \Psi^{(v_1 \combine \flip{p_2})},x \mcol A \cb (\Gamma \vdash M) \ok \\\\
            \Psi^{p_1} \vdash {a_1} : A \\\\
            \Psi^{p_2} \vdash {a_2} : A \\\\
            \Psi^{(v_1 \combine \flip{p_2} \combine {p_2})} \cb (\Gamma \vdash M)[a_2/x] }
          {\Psi^{v} \cb (\Gamma, \mor{A} (x,a_2) \vdash M)[a_1/x]}
