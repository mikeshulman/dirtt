...
\begin{mathpar}
	\inferrule{
		...
		\Delta \cb \Gamma, \esh^{x:A} M \types C
	}\and
\end{mathpar}
Right rule:
%\begin{mathpar}
...
%		r
%	}{
%		\Delta, \Delta_a, \Delta_b \cb \Gamma \types \esh_{}
%	}
%\end{mathpar}
We should probably base the right rule on the following function:
\begin{equation}
	\lambda f.\lambda z.(z, fzz) : [\esh_x \esh_y G(x^-, x^+, y^-, y^+)] \to [\esh_z \esh^w G(z^-, w^+, w^-, z^+)]
\end{equation}
...
\begin{mathpar}
	\inferrule{
		...
		\Delta, z : A \cb \Gamma \types \esh^{w:A} M
	}\and
\end{mathpar}
...
\begin{equation}
	\inferrule{
		...
		Z : \Set \cb z : \check Z \types \esh^{W:\Set} \hat W \otimes \hom_{\Set}(\check W, \hat Z)
	}
\end{equation}
...
\begin{mathpar}
	\inferrule{
		...
		\Delta \cb \Gamma \types \esh_{x:A} M
	}\and
\end{mathpar}
The left rule can based on the function
\begin{equation*}
	[\esh_x \esh_y (F(x^-, x^+, y^-, y^+) \to G(x^-, y^+))] \to \esh_{z}[(\esh_w F(z^-, w^+, w^-, z^+)) \to G(z^- , z^+)]
\end{equation*}
...
\begin{mathpar}
	\inferrule{
		...
		\Delta, z : A \cb \Gamma, \esh_{w : A} M \types C
	}
\end{mathpar}
