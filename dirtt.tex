...
\begin{center}
  ...
  \end{array}$
\end{center}
Here $\Delta\splits \Psi_1,\Psi_2$ means that each variable $x$ in the unsigned context $\Delta$ becomes a pair $\hat x, \check x$ of opposite variance in the signed context $\Psi_1,\Psi_2$ (see the rules below).
And as above, the notation $\flip{\Psi_1}$ means that we reverse the variances of the variables in $\Psi_1$.
For example,
$x:A \splits \check x : \m A, \hat x \pcol A$, and $\flip{(\check x \mcol A)} = \check{x} \pcol A$, so a well-formed judgement could have 
$\check x \pcol A \types \Gamma \ctx$ and $\hat x \pcol A \types A \type$. This says that $\check{x}$ is allowed to occur covariantly in $\Gamma$, which counts as a negative occurrence overall, because $\Gamma$ is itself in a contravariant position.

We will write
\[ \Delta \types \Gamma\ctx, N\type \]
to mean the conjunction of the preconditions for $\Delta\cb\Gamma\types N$ to make sense:
\begin{mathpar}
  \Delta \splits \Psi_1,\Psi_2 \and
  \flip{\Psi_1} \types \Gamma \ctx\and
  \Psi_2 \types M \type
\end{mathpar}
...
Thus we always have $\unsigned{\Psi} = \unsigned{(\dual\Psi)}$ and $\unsigned{\Psi} \splits \Psi,\dual\Psi$.
...
If $a$ is a category-term in context $\Psi$, we will also write $\dual{a}$ for the result of interchanging hats and checks in $a$.
...
\begin{mathpar}
  ...
  \inferrule{\Delta,x:A \types \Gamma\ctx, M\type\\ \Psi_a \types a:A \\ \Psi_b \types b:B \\ \Delta,x:A \cb \Gamma \types M}{\Delta,\unsigned{\Psi_a},\unsigned{\Psi_b}\cb \Gamma[\dual{a}/\check x,\dual{b}/\hat{x}],\mor A(a,b) \types M[\dual{a}/\check x,\dual{b}/\hat{x}]}\and
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
    \Delta,x:A \types \Gamma\ctx, M\type\\
    \Phi_a \types a:A\\
    \Delta\cb \Gamma[\dual b/\check x,b/\hat x] \types M[\dual b/\check x,b/\hat x]
  }{
    \Delta,\unsigned{\Phi_a}\cb \Gamma[\dual a/\check x,b/\hat x],\mor A(a, \dual b) \types M[\dual a/\check x,b/\hat x]
  }\and
  \inferrule{
    \Delta,x:A \types \Gamma\ctx, M\type\\
    \Phi_b \types b:B\\
    \Delta\cb \Gamma[\dual a/\check x,a/\hat x] \types M[\dual a/\check x,a/\hat x]
  }{
    \Delta,\unsigned{\Phi_b}\cb \Gamma[\dual a/\check x, b/\hat x],\mor A(a, \dual b) \types M[\dual a/\check x, b/\hat x]
  }\and
\end{mathpar}
