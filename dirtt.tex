...
\red{
  \inferrule{
  	...
  	\erasevar{\Psi} \cb M[\flip{\Psi}/\Psi] \types M
  }
}
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
  	x:\p A, y:\m B, z:\p C \types M(x,y,z)\type
  }{
...
\begin{mathpar}
  \inferrule{
  	...
  	\Delta',\erasevar\Psi \cb \Gamma[a/x] \types M[a/x]
  }
\end{mathpar}
}
...
\begin{mathpar}
  ...
  \inferrule{
  	...
  	\erasevar\Psi \cb \ec \types \mor A(a,a)
  }}\and
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
    (x:A) \in \joinvar{\flip{\Psi_\Gamma}, \Psi_M} \\\\
    \Psi_\Gamma \types \Gamma\ctx \\ \Psi_M \types M\type \\\\
    \Psi_a \types a:A \\ \Psi_b \types b:A \\
    \joinvar{\flip{\Psi_\Gamma}, \Psi_M} \cb \Gamma \types M
  }{
  	\joinvar{\flip{\Psi_\Gamma}, \Psi_M}\setminus\{x : A\},\erasevar\Psi_a,\erasevar\Psi_b \cb \Gamma[a/\check x,b/\hat x],\mor A(a,b) \types M[a/\check x,b/\hat x]
  }\and
\end{mathpar}
}
...
\begin{mathpar}
  \inferrule{
    ...
    \Psi_M,\Phi_M,\Phi_M',\hat x:\p A,\check x:\m A \types M\type \\\\
...
\begin{mathpar}
  \inferrule{
    ...
    \Psi_M,\Phi_M,\flip{{\Phi_M'}},\hat x:\p A,\check x:\m A \types M\type \\\\
    \joinvar{\Psi_M, \flip{\Psi_\Gamma}},\erasevar{\Phi_\Gamma}, \erasevar{\Phi_M},\erasevar{{\Phi_\Gamma'}}, \erasevar{{\Phi_M'}} \cb \Gamma \types M[a/\hat x,a[\Phi_\Gamma'/\Phi_\Gamma, \Phi_M'/\Phi_M]/\check x]
  }{
    \joinvar{\Psi_M, \flip{\Psi_\Gamma}},\erasevar{\Phi_\Gamma}, \erasevar{\Phi_M} \cb \Gamma[\Phi_\Gamma/\Phi_\Gamma'] \types \coend^{x:A} M[\Phi_M/\Phi_M']
  }\and
\end{mathpar}
}
Co-Yoneda lemma:
