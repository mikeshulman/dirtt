...
\begin{equation}\label{eq:triangle}
  \inferrule{\inferrule{\inferrule{\eta}{x:A \cb \ec\types \mor A(x,gf(x))}}
  ...
  \inferrule{\text{(composition)}}{x:A \cb \mor B(fgf(x),f(x)), \mor B(f(x),fgf(x)) \types \mor B(f(x),f(x))}}
...
On the upper left we are hoping to get $f(\eta)$\dots
