...
Without substitutions included, it looks like this:
\begin{equation}\label{eq:coend-right-nosub}
  \inferrule{
...
In this case, it can be used to derive the functoriality of coends:
\begin{mathpar}
  \inferrule{
    \inferrule{
...
Finally, here is the version of the right rule with substitutions included:
\begin{mathpar}
  \inferrule{
    \Psi,\dual\Psi,\check x \mcol A, \hat y\pcol A \cong \Psi_\Gamma,\Psi_M\\
    \Psi_\Gamma \types \Gamma\ctx \\
    \Psi_M, \hat x\pcol A, \check y\mcol A \types M \type\\
    \Psi_a \types a:A \\
    \unsigned\Psi, x : A, y : A \cb \Gamma \types M
  }{
    \unsigned\Psi, \unsigned{\Psi_a} \cb \Gamma[a/\hat y,\dual a/\check x] \types \coend^{w:A} M[a/\hat y,\dual a/\check x,\hat w/\hat x, \check w/\check y]
  }  
\end{mathpar}
(We are unable to use the single-judgment version~\eqref{eq:ctx-type} of the preconditions, since we need to ensure that $\hat x$ and $\check y$ appear in $M$ rather than in $\Gamma$.)

\gray{Here is what was written as the right rule before.  I'm still unable to make much sense of it, in either form; the above version seems like what we would get by straightforwardly adding a substitution to~\eqref{eq:coend-right-nosub}.)
\begin{mathpar}
  \inferrule{
