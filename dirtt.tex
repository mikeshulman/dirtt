...
\begin{mathpar}
  \inferrule{
    \Delta \iso \Delta' \\
    \Delta \splits (\flip{\Psi_1},\Psi_2),\Psi_a \\
    \Delta' \psplit \flip{\Psi_a'},(\Psi_1',\flip{\Psi_2'}) \\
    \Delta_M \splits \Psi_M,\flip{\Psi_{\Gamma}} \\\\
    \Psi_a \types a:A \\ \Psi_{\Gamma_M},\Psi_1,\Psi_1' \types \Gamma\ctx \\
    \Psi_M,\Psi_2,\Psi'_2,\hat x:\p A,\check x:\m A \types M\type \\\\
    \Delta_M,\Delta,\Delta' \cb \Gamma \types M[a/\hat x,a[\flip{\Psi_a'}/\Psi_a]/\check x]
  }{
    \Delta_M,\Delta \cb \Gamma[\Psi_1/\Psi'_1] \types \coend^{x:A} M[\Psi_2/\Psi'_2]
  }\and
\end{mathpar}
Co-Yoneda lemma:
\begin{mathpar}
  \inferrule{
    \inferrule*{
      \inferrule*{
        ...
        \Delta_M,\Delta,\Delta' \cb M(a) \types M(a)
      } \\
      \inferrule*{
        \vdots
      }{
        \Delta_M,\Delta,\Delta' \cb \cdot \types \hom_A(a',a')
      }
    }{
      \Delta_M,\Delta,\Delta' \cb M(a) \types M(a) \otimes \hom_A(a',a')
    }
  }{
    \Delta_M,\Delta \cb M(a) \types \coend^{x:A} M(\hat x)\otimes \hom_A(\check x,a)
  }\and
\end{mathpar}
and conversely
\begin{mathpar}
  \inferrule{
    \inferrule{
      \Delta_M,\Delta \cb M(a) \types M(a)
    }{
      \Delta_M,\Delta,x:A \cb M(\hat x) \otimes \hom_A(\check x,a) \types M(a)
    }
  }{
    \Delta_M,\Delta \cb \coend^{x:A} M(\hat x) \otimes \hom_A(\check x,a) \types M(a)
  }
\end{mathpar}
	...
	\inferrule{
		\inferrule{
			\inferrule{
				...
				X : \Set \cb x : \check X \types \hat X
			}
			\qquad
			\inferrule{
				Y :^+ \Set \types Y : \Set
			}{
				Y : \Set \cb \cdot \types \hom_{\Set}(\check Y, \hat Y)
			}
		}{
			X : \Set, Y : \Set \cb x : \check X \types \hat X \otimes \hom_{\Set}(\check Y, \hat Y)
		}
	}{
		Z : \Set \cb z : \check Z \types \coend^{W:\Set} \hat W \otimes \hom_{\Set}(\check W, \hat Z)
	}
\end{equation}
...
\begin{mathpar}
	\inferrule{
		\Psi_A, x:\pm A \types M \type \\
	}{
		\Psi_A \types \coend_{x:A} M \type
	}\and
\end{mathpar}
Right rule:
\begin{mathpar}
