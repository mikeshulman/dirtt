...
\begin{mathpar}
  \inferrule{
    ...
    \Delta_M,\Delta_N \cb \Gamma_M,\Gamma_N \types t_m\otimes t_n : M\otimes N
  }\and
\end{mathpar}
\begin{mathpar}
  \inferrule{
...
\end{mathpar}
with $\text{``\texttt{let $m\otimes n$ = $t_m\otimes t_n$ in $c$}''}=c[t_m/m,t_n/n]$ and $\text{``\texttt{let $m\otimes n$ = $t$ in $c[m\otimes n/z]$}''}=c[t/z]$.
...
\begin{mathpar}
  \inferrule{
    ...
    \Delta,y:A \cb \Gamma[y/x'] \types \langle y,t_m[y/x,y/x']\rangle : \coend^{x:A} M[y/x']
  }\and
\end{mathpar}
\begin{mathpar}
  \inferrule{
...
\end{mathpar}
with $\text{``\texttt{let $\langle x,m\rangle$ = $\langle y,t_m\rangle$ in $c$}''}=c[y/x,t_m/m]$ and $\text{``\texttt{let $\langle x,m\rangle$ = $t$ in $c[\langle x,m\rangle/z]$}''}=c[t/z]$.
  ...
  \inferrule{
    \inferrule{
      \inferrule{
        ...
        x:A,y:A \cb n:N(\check x), t:\hom_A(\hat x,\check y) \types J(x.y.N(x),x.n,x,y,f):N(\hat y)
      }
      }{
      x:A,y:A \cb t:N(\check x)\otimes\hom_A(\hat x,\check y) \types
        \text{``\texttt{let $n\otimes f$ = $t$ in $J(x.y.N(x),x.n,x,y,f)$}''}:N(\hat y)
      }
    }{
    y:A \cb q:\coend^{x:A} N(\check x)\otimes\hom_A(\hat x,\check y) \types
      \text{``\texttt{let $\langle x,t\rangle$ = $q$ in \{let ...\}}''}:N(\hat y)
    }
\end{mathpar}
and conversely
\begin{mathpar}
  \inferrule{
    \inferrule{
      ...
      x:A,x':A \cb n:N(\check x) \types n\otimes\id(x') : N(\hat x)\otimes\hom_A(\check x',\hat x')
    }
    }{
    y:A \cb n:N(\check y) \types \langle y,n\otimes\id(y)\rangle : \coend^{x:A}N(\hat x)\otimes\hom_A(\check x,\hat y)
  }
\end{mathpar}
which we can check are inverse:
\begin{align*}
  &\text{``\texttt{let $\langle x,t\rangle$ = $\langle y,n'\otimes\id(y)\rangle$ in \{let $n\otimes f$ = $t$ in $J(x.y.N(x),x.n,x,y,f)$\}}''} \\
  &= \text{``\texttt{let $n\otimes f$ = $n'\otimes\id(y)$ in $J(x.y.N(x),x.n,y,y,f)$}''} \\
  &= J(x.y.N(x),x.n',y,y,\id(y)) \\
  &= n'
\end{align*}
