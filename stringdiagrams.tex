...
\begin{mathpar}
	\inferrule{
		...
		\Psi, \strs s, \Phi' \strto \Phi \cb \Gamma \vdash \id : \mor A(a[\rho], a)
	}\and
\end{mathpar}
Left rule:
\begin{mathpar}
...
\begin{mathpar}
	\inferrule{
		...
		\Psi, \strs s, \Phi'_b \strto \Phi_b \cb \Gamma[b/z], f : \mor A(a, b[\rho]) \vdash J(f, m) : M[b/z]
	}\and
\end{mathpar}
...
\begin{mathpar}
	\inferrule{
		...
		\Psi, \strs s, w \strto z : A \cb \Gamma \vdash (z, m) : \coend^{x \strto y : A} M
	}\and
\end{mathpar}
...
\begin{mathpar}
	\inferrule{
		...
		\Psi, \strs s, \Phi_w \strto \Phi_x, \Phi_y \strto \Phi_z \cb \Gamma \vdash m : M[a[\rho_x]/x, a[\rho_y]/y]
	}{
		\Psi, \strs s, \Phi_w \strto \Phi_z \cb \Gamma \vdash (a, m) : \coend^{x \strto y : A} M
	}\and
\end{mathpar}
...
\todoi{Here we have a term that refers to category variables, which I wanted to avoid.}
...
\begin{mathpar}
	\inferrule{
		...
		\flip{\Psi_\Gamma}, \flip{\Psi_M}, \Psi_N, \strs s \cb \Gamma, q : \coend^{x \strto y:A} M \vdash \letexp{(?, m)}{q}{n} : N
	}\and
\end{mathpar}
\todo{Update notation:}
...
\begin{mathpar}
	\inferrule{
		\inferrule{
			\inferrule{\,}{\check x \strto \hat y : A \cb t : T(\check x) \vdash t : T(\hat y)} \\
			\inferrule{\,}{\check y \strto \hat x : A \cb \cdot \vdash \id : \mor{A}(\check y, \hat x)}
		}{
			\check x \strto \hat y : A, \check y \strto \hat x : A \cb t : T(\check x) \vdash t \otimes \id : T(\hat y) \otimes \mor{A}(\check y, \hat x) 
		}
	}{
		\check x \strto \hat x : A \cb t : T(\check x) \vdash (\hat x, t \otimes \id) : \coend^{\hat y \strto \check y} T(\hat y) \otimes \mor{A}(\check y, \hat x)
	}
\end{mathpar}
