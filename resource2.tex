...
\[
\begin{array}{lll}
x \vcol r A \in (x \vcol {b} A') & \text{ iff } & r = b \text{ and } A = A' \\
...
x \vcol{b} A \in \Delta, x \vcol{\flip{b}} A \in \Delta$), and $\Phi$
for a context where no variable is $\pms$ ($\forall x \vcol{b} A \in
\Phi, \neg (x \vcol{\flip{b}} A \in \Phi)$).  Note that $\Phi \combine
...
\begin{mathpar}
  ...
  \inferrule{
    [  (\Delta_1 \combine x \pmcol A \combine y \pmcol A) \equiv \Psi_1 \combine (\Psi_1' \combine x \pcol A \combine y \mcol A) ]\\\\
        ...
        \unif{\Psi_3 \combine \Psi_3'}{\Psi_2 \combine \Psi_2'}{\rho_3'^{-1}[\rho]}{(\Delta_0 \vdash \rho_0)}
  }{
    \unif{\Psi_1 \combine (\Psi_1' \combine x \pcol A \combine y \mcol A)}
         {\Psi_2 \combine (\Psi_2' \combine z \pmcol A)}
...
\inferrule{ }
          {\cdot \vdash \cdot : \cdot}
\and
\inferrule{ \Psi^b \vdash a : A}
          { \Psi \vdash a/x^b : x \vcol b A}
\and 
\inferrule{ \Psi_1 \vdash \rho_1 : \Psi_1'  \\
            \Psi_2 \vdash \rho_2 : \Psi_2' \\
           }
...
\section{Explicit Substitution Rules}
...
\qquad
\inferrule{{\Phi'}\complete  \cb \Gamma \vdash M \\
            \Phi \types \theta : \Phi'
           }
...
\inferrule
    ...
    {\Phi_1 \combine \Phi_2 \cb M_1 , M_2 \types M_1 \otimes M_2}
...
\qquad
\inferrule{
  \Psi \combine x \pmcol A \cb \Gamma \types M
}{
...
\inferrule{\Phi \types N \type }
          {\flip \Phi \combine \Phi \cb N \types N}
\\
\end{mathpar}
