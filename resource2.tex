...
\newcommand{\Jb}[7]{\mathtt{transport}\; #1\coloneqq #2@#3 \;\mathtt{to}\; #4\coloneqq#5 \;\mathtt{via}\; #6 \;\mathtt{in}\; #7}
\newcommand{\Jd}[5]{\mathtt{transport}\; #1\coloneqq(#2,#3) \;\mathtt{via}\; #4 \;\mathtt{in}\; #5}
\mprset{flushleft}
...
We seem to need several special cases of elim terms for hom types.

\textsc{Hom Left Rule-D}: This is the simplest case.
\begin{mathpar}
  \inferrule{
    ...
    \Phi_2 \types a_2:A \\\\
    \Delta \equiv \Psi,\Psi' \\
    \Delta, x\pmcol A \cb \Gamma \types m:M[x/y\p]
    }{
    \Delta,\Phi_1\complete,\Phi_2\complete \cb \Gamma, f:\mor{A}(a_1,a_2) \types \\\\
    \quad (\Jd{x}{a_1}{a_2}{f}{m}):M[a_1/x\m,a_2/y\p]
  }
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
    ...
    \Phi_1 \types a_1:A \\
    \Phi_2 \types a_2:A \\\\
    \Phi\complete, x\pmcol \cb \ec \types \id(F):\mor{B}(\flip{F},F)
    }{
...
by
\[
  \mathtt{app}(F,f) \equiv (\Jd{x}{a_1}{a_2}{f}{\id(F)})
\]

\textsc{Hom Left Rule-B}: Without substitutions build in, this looks like
\begin{mathpar}
  \inferrule{
    \flip{\Psi_1} \types \Gamma \ctx \\
    \flip{\Psi_2}, x\pcol A \types N \type \\
    \Psi', y\pcol A \types M \type \\\\
    \Delta\equiv\Psi_1,\Psi_2,\Psi' \\
    \Delta, z\pmcol A \cb \Gamma, r:N[z/x\p] \types m:M[z/y\p]
    }{
    \Delta, x\pmcol A, y\pmcol A \cb \Gamma, r':N, f:\mor{A}(x,y) \types \\\\
    \quad (\Jb{r}{r'}{x}{z}{y}{f}{m}):M
  }
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
    ...
    \Psi_1', x\pcol A \types N \type \\
    \Psi_2', y\pcol A \types M \type \\\\
    \Psi_3 \types a_1:A \\
    \Phi \types a_2:A \\
    \flip{\Psi_{21}} \types \rho:\Psi_1' \\\\
    \Psi_1,\Psi_1',\Psi_3 \cb \Gamma_1 \types n:N[a_1/x\p] \\\\
    \Psi_{21},\Psi_{22},\Psi_2',z\pmcol A
      \cb \Gamma_2, r:N[\rho,z/x\p] \types m:M[z/y\p] \\\\
    \unif{(\Psi_1,\Psi_3)\combineU\Psi_1'}
      {(\Psi_{22},\Psi_2')\combineU\Psi_{21}}{\rho}{(\Delta_0\types\rho_0)}
    }{
    \Delta_0,\Phi\complete \cb (\Gamma_1,\Gamma_2,\Gamma_3,f:\mor{A}(a_1,y) \types \\\\
      \quad (\Jb{r}{n}{a_1}{z}{a_2}{f}{m}):M)[\rho_0,a_2/y\p,\flip{a_2}/y\m]
  }
\end{mathpar}
This has computation rules
\begin{gather*}
  (\Jb{r}{n}{a}{z}{a}{\id(a)}{m}) \equiv m[a/z,n/r] \\
  (\Jb{r}{r}{x}{z}{y}{f}{m[z/x,,z/y,\id(z)/f]}) \equiv m
\end{gather*}

Using this, we can define a $J$ term as a special case, giving the covariant functoriality of types:
\begin{mathpar}
  \inferrule{
    \Phi \types a_2:A \\
    \Delta \cb \Gamma \types m:M[a_1/x\p]
    }{
    \Delta,\Phi\complete \cb \Gamma, f:\mor{A}(a_1,a_2) \types J\p(m,a_1,a_2,f):M[a_2/x\p]
  }
\end{mathpar}
This is defined by
\[
  J\p(m,a_1,a_2,f) \equiv (\Jb{r}{m}{a_1}{x}{a_2}{f}{r})
\]
and satisfies
\[
...
\textsc{Hom Left Rule-A}: This splits into two cases, depending on whether the $x$ and $y$ in
$\Gamma'$ appear in the same or in different types. Without substitutions built in, we have
\begin{mathpar}
  \inferrule{
    \flip{\Psi_1} \types \Gamma \ctx \\
    ...
    \Delta,z\pmcol \cb \Gamma, r:N_1[z/x\p], s:N_2[z/y\m] \types m:M
    }{
    \Delta,x\pmcol A,y\pmcol A \cb \Gamma, r':N_1, s':N_2, f:\mor{A}(x,y) \types \\\\
      \quad(\Jaa{r}{r'}{x}{s}{s'}{y}{z}{f}{m}):M
  }
\end{mathpar}
\begin{mathpar}
  \inferrule{
...
\begin{mathpar}
  \inferrule{
    ...
    \Delta_0 \cb (\Gamma_1,\Gamma_2,\Gamma_3,f:\mor{A}(a_1,a_2)
      \types (\Jaa{r}{n_1}{a_1}{s}{n_2}{a_2}{x}{f}{m}):M)[\rho_0]
  }
\end{mathpar}
\begin{mathpar}
  \inferrule{
    ...
    \Delta_0 \cb (\Gamma_1,\Gamma_2,\Gamma_3,f:\mor{A}(a_1,a_2)
      \types (\Jab{r}{n}{a_1}{a_2}{x}{f}{m}):M)[\rho_0]
  }
\end{mathpar}
