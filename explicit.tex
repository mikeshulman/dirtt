...
\title{A directed type theory for formal category theory --- explicit substitution rules}
...
\begin{mathpar}
\begin{array}{l}
\Psi ::= \cdot \mid \Psi_1,\Psi_2 \mid x \pcol A \mid x \mcol A \\
\Delta ::= \cdot \mid \Delta_1,\Delta_2 \mid x \pmcol A \\
\Phi ::= \cdot \mid \Phi_1,\Phi_2 \mid x \pcol A \mid x \mcol A \mid x \pmcol A  \\
\\ 
\Gamma ::= \cdot \mid \Gamma, M\\
J ::= \Gamma \types N \mid J[\theta]
\end{array}
\end{mathpar}

We silently coerce a $\Psi$ or a $\Delta$ to a $\Phi$.  

For every $\Psi$, there is another $\Psi$-context $\Psi\m$ defined by mapping the function 
\[
\begin{array}{lll}
\pcol  & \mapsto & \mcol \\
\mcol  & \mapsto & \pcol \\
...
For every $\Psi$, there is a $\Delta$-context $\Psi\pm$ defined by mapping the function 
\[
\begin{array}{lll}
\pcol  & \mapsto & \pmcol \\
\mcol  & \mapsto & \pmcol \\
...
Judgements and their meaning (described as if it's normal categories;
TODO extend with enriched/equipment terms).  

\begin{itemize}

\item A context $\Phi$ represents a product category, or a bunch of
  categories if we have multicategories in mind, where $x \pcol A$ means
  $A$, $x \mcol A$ means $A\op$, $x \pmcol A$ means $A\op \times A$.

\item $\Phi \types \theta : \Phi'$ means a functor from $\Phi$ to
  $\Phi'$.  Warning for multicategories: not every thing will be
  literally a list arrows, but maybe everything could normalize to one?

\item $\Psi \types a : A$ means a functor from $\Psi$ to $A$ (note that
  only signed variables are allowed here).  

\item $\Phi \types J \ok$ means $J$ is a functor from $\Phi \to \Set$

\item $\Psi \types \Gamma \ctx$ means $\Gamma$ is a functor from $\Psi$
  to $\Set$ (note that only signed variables are allowed here)

\item $\Psi \types M \type$ means $M$ is a functor from $\Phi$ to $\Set$
  (note that only signed variables are allowed here)

\item $\Delta \cb J$, where $\Delta \types J \ok$, means an element of
  the end of $J$ (which makes sense because a $x \pmcol A$ assumptions
  stands for two assumptions $A\op,A$).
\end{itemize}

We enforce the invariant that all variables in a context are distinct,
and we treat contexts and substitutions up to
associativity/commutativity.  We write $\Psi_*$ and $N_*$ etc. for
$\alpha$-renaming all variables in a consistent way (e.g. replacing $x$
with $x^*$).  This is used when we union contexts $(\Psi_1,\Psi_2)$ and
$\Psi_1$ and $\Psi_2$ share variables.  In de Bruijn form this wouldn't
be necessary because an index into the combined context would include a
left/right bit.  
...
\inferrule{ }
          { \Phi \types 1_\Phi : \Phi}
\and
\inferrule{ \Phi \types \theta : \Phi' \\
            \Phi' \types \theta' : \Phi''}
          { \Phi \types \theta;\theta' : \Phi'' }
\and
\inferrule{ \Phi_1 \types \theta_1 : \Phi_1' \\
            \Phi_2 \types \theta_2 : \Phi_2' 
          }
          { \Phi_1,\Phi_2 \types (\theta_1,\theta_2) : \Phi_1',\Phi_2' }
\and
\inferrule{ \Psi \types a : A }
          { \Psi \types a/x\p : (x \pcol A) }
\and
\inferrule{ \Psi\m \types a : A }
          { \Psi \types a/x\m : (x \mcol A) }
\\ 
\inferrule{ \Psi \types a : A }
          { \Psi\pm \types a/x\pm : (x \pmcol A) }
\and
\inferrule{ \Psi\m \types a : A \\ (\text{is this necessary?})}
          { \Psi\pm \types a/x\mp : (x \pmcol A) }

\inferrule{ }
          { (x \pmcol A) \types (x \csplit y,z) : (y \mcol A , z \pcol A)}
\\
\end{mathpar}

There will then be lots of equations on substitutions.  For example, when $\Psi \types a : A$ we have 
\[
\Psi\pm \types ((a/x\pm);(x \csplit \hat x, \check x)) : (\check x :\m A, \hat x:\p A)
\] which is like the $(\hat a/\hat x, \check a /
\check x)$ substitution.  This should be equal to first splitting
$\Psi$.  

It should be admissible from these that 
\[
\inferrule{ }
          {\Psi\pm \types \Psi\pm \csplit (\Psi_*\m,\Psi) : (\Psi_*\m,\Psi)}
\]
by comma-ing together an $x \csplit x^*,x$ or $x \csplit x,x^*$ for each
assumption in $\Psi$ (depending on its variance in $\Psi$), and then
(silently) exchanging variables.
...
\begin{mathpar}
  \inferrule{
    ...
    \Psi_1\m,\Psi_2 \types (\Gamma \vdash M) \ok
  }
  \and
  \inferrule{
    \Phi' \types J \ok \\
...
\subsection*{Sequent Structural Rules}

\begin{mathpar}
\inferrule{\Psi \types N \type }
          {\Psi\pm \cb (N_* \types N) [ \Psi\pm \csplit (\Psi_*\m,\Psi)]}

\inferrule{\Delta' \cb J \\
...
\begin{mathpar}
\inferrule{ }{x\mcol A, y\pcol A \types \mor A(x,y) \type}\and

\inferrule{ }
          {x \pmcol A \cb (\ec \types \mor{A}(\check x,\hat x)) [ x \csplit \check x, \hat x ] }

\inferrule{\Delta,x \pmcol A \cb (\Gamma \vdash M) [ \theta, x \csplit (\check x,\hat y) ]}
          {\Delta,x \pmcol A,y \pmcol{A} \cb (\Gamma, \mor{A} (\hat x,\check y) \vdash M)
            [\theta,x \csplit (\check x , \hat x), y \csplit (\check y, \hat y)]}
...
Need to think more about based path induction.  

%% For the morphism induction rule, in the conclusion, we have 
%% \[
%% \Delta, x \pmcol A \vdash (\theta,x \csplit
%%   \check x , \hat x) : (\Phi,\check{x} :\m A, \hat{x} :\p A)
%% \]
%% and 
%% \[
%% (\Phi,\check{x} :\m A, \hat{x} :\p A) \types (\Gamma,\mor{A}(a,\check{x}) \vdash M) \ok
%% \]
%% so $\Phi$ should be $\Psi\m_\Gamma,\Psi_a,\Psi_M$
%% \begin{mathpar}
