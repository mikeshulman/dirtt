...
\item A context $\Phi$ represents a product category, or a bunch of
  ...
  and $x \mpcol A$ means $A \times A\op$.
...
\begin{mathpar}
\inferrule{x \pcol A \in \Psi_1}
          {x \pcol A \in (\Psi_1,\Psi_2)}
\and
\inferrule{x \pcol A \in \Psi_2}
          {x \pcol A \in (\Psi_1,\Psi_2)}
\and
\inferrule{ }
          {x \pcol A \in x \pcol A}

\inferrule{ x \pcol A \in \Psi }
          {\Psi \vdash x : A}
\\ 
\end{mathpar}
...
\inferrule{ }
          {\Psi\pm \types \Psi\pm \csplit (\Psi\m,\Psi) : (\Psi\m,\Psi)}
\\
\end{mathpar}
...
\subsection*{Judgement Well-formedness}
...
\begin{mathpar}
  \inferrule{
    ...
    \Psi_1\m,\Psi_2 \types (\Gamma \vdash M) \ctx
  }
  \and
  \inferrule{
    \Phi' \types J \ok \\
...
\inferrule{\Psi \types N \type }
          {\Psi\pm \cb (N \types N) [ \Psi\pm \csplit (\Psi\m,\Psi)]}
